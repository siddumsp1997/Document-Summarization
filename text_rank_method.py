...
# module to retrieve all the words (excluding stop words) and removing
# special characters
...
#power_method() used in text rank algorithm
def power_method(cosine_matrix, n, e):

	#Damping constant d
	d = 0.01
	#eigen vector matrix pt
	pt = numpy.zeros(n)

	MT = cosine_matrix.transpose()

	d_one_n = numpy.zeros(n)
	...
	while True:
		...
		pt = d_one_n + (1-d)*(numpy.matmul(MT,pt_prev)).transpose()

		delta = 0.0 

		for i in range(0,n):

			delta += math.pow(pt[i] - pt_prev[i], 2)
...
#Module for summary generation (using TextRank algorithm)
def summary_generation_text_rank(graph, threshold, node_list):
	...
	respect_values = numpy.zeros(n)

	for i in range(0,n):

		for j in graph[i].keys():

			cosine_matrix[i][j] = 1.0 / len(graph[i]) 


	#print cosine_matrix		


	respect_values = power_method(cosine_matrix, n, e)
	...
	for i in range(0,n):
		...
		tmp_pair = (respect_values[i], i)

		node_info.append(tmp_pair)

	#arrange in descending order of respect
	node_info.sort(reverse = True)
	...
	while True:
		...
		#print( "Sentence :  " + node_list[node_info[i][1]].sentence +"\n\n")

		word_count += len(node_list[node_info[i][1]].sentence.strip().split(" "))

		if word_count >= 250:

			break

		final_sentences_index.append(node_info[i][1])	
...
if __name__ == '__main__':  
	...
	for index in range(0,5):
		...
		for threshold in thresholds:
			...
			final_sentence_index_list = summary_generation_text_rank(graph, threshold, node_list)
